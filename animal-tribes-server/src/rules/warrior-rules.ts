...
export const signUpRules = yup.object().shape({
  ...
  warriorname: yup
    ...
    .test('uniqueWarrior', 'This warrior already exists', async warriorname => {
  ...
  password: yup
    ...
    .matches(
      /[a-zA-Z0-9@!#%]/,
      'Password can contain only Latin letters, numbers and/or [@, !, #, %].'
    ),
});
...
export const tribeRules = yup.object().shape({
  tribe: yup
    ...
    .test(
      'uppercaseCheck',
      'Tribe can only accept uppercase letters',
      tribe => {
